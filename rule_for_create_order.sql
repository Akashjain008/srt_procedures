CREATE OR REPLACE FUNCTION public.rule_for_create_order(ordersarray text)
 RETURNS text
 LANGUAGE plpgsql
AS $function$
...
Begin
	...
	LOOP
		...
		FOR recordCheck IN (SELECT * FROM tr_rule where is_active=true and db_level=true and event_type ='createOrder' order by 1)
		LOOP			
					
			--=======check repair Service Partner Id rule========
			if (recordCheck."code" = 'SRTO20004') then
